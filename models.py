...
def create_modules(module_defs):
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            yolo_layer = YOLOLayer(anchors, num_classes) # yolo layer를 구서
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path):
        ...
        self.module_defs = parse_model_config(config_path) # cfg 파일을 파싱하여 리스트로 구성
    ...
    def load_darknet_weights(self, weights_path):
        ...
        with open(weights_path, "rb") as f:
            ...
            header = np.fromfile(f, dtype=np.int32, count=5)  # 앞 5자리는 header value임
            self.header_info = header  # Needed to write header when saving weights
            self.seen = header[3]  # number of images seen during training
            weights = np.fromfile(f, dtype=np.float32)  # The rest are weights

        # Establish cutoff for loading backbone weights
        cutoff = None # weight가 총 몇개의 layer로 구성되어있는지 카운트하기위한 설정
        ...
        ptr = 0
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if i == cutoff: # 마지막 레이어에 도달하면 break
                break
            if module_def["type"] == "convolutional":
                conv_layer = module[0]
